...
# 1번째(Index 0): 100점, 2번째: 90점, ..., 10번째: 10점
...
async def run_evaluation_process(model, device, audio_np, expected_word, language="en", difficulty=3):
  ...
  best_candidate = candidates[0] # 기본값은 정답 단어
  best_avg_score = -1
  best_result_aligned = None
  ...
  try:
    ...
    for cand in candidates:
      ...
      if avg_score > best_avg_score:
        ...
        best_candidate = cand
        best_result_aligned = result_aligned
        print(f"    ⭐ New best match: '{cand}'")

    print(f"✅ Final Choice: '{best_candidate}' (Confidence: {best_avg_score:.4f})\n")

    # 2. 최종 결과 정리
    actual_text = best_candidate
    
    # 3. 발음 평가 서비스 호출
    evaluation = evaluate_pronunciation(
      expected_word, 
      actual_text, 
      word_list=candidates, # 전체 후보군 리스트 전달
      language=language,
      difficulty=difficulty, # 난이도 전달
      aligned_segments=best_result_aligned.get("segments") if best_result_aligned else []
    )
