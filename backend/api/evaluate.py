...
# --------------------------------------------------
async def run_evaluation_process(model, device, audio_np, expected_word):
  """
  ...
  # 1. WhisperX 내부의 Faster-Whisper 모델을 직접 사용하여 정밀 전사
  ...
  try:
    ...
    evaluation = evaluate_pronunciation(expected_word, actual_text, word_list)
...
async def evaluate(request: Request, file: UploadFile = File(...), expected: str = Form(...)):
  ...
  try:
    ...
    # 3. 공통 함수 호출
    result = await run_evaluation_process(model, device, audio_np, expected)
