...
# OpenAI Whisper 원본이 아니라 faster-whisper 를 사용 중
# 더 빠르고 메모리 효율이 좋아 서버 운영에 적합. 실서비스에서 많이 사용
      ...
      if "bytes" in message:
        ...
        if len(audio_buffer) > 32000 * 2:  # 약 2초
          ...
          segments, info = model.transcribe(
            audio_np,
            beam_size=5, # 발음 평가를 위해 beam_size를 높여 정확도를 높임
            language="en", # 영어로 고정 (제시어가 영어이므로)
            initial_prompt=target_word, # 제시어 우선 인식
          )

          end_time = time.time()
          logger.info(f"📊 Whisper 인식 시간: {end_time - start_time:.2f}초")
